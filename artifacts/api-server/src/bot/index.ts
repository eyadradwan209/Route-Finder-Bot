...
async function handleAddAirport(interaction: ChatInputCommandInteraction) {
  ...
  if (already.length > 0) parts.push(`Already listed: **${already.join(", ")}**`);
...
async function handleRemoveAirport(interaction: ChatInputCommandInteraction) {
  ...
  if (removed) {
    ...
    await interaction.reply(
      `**${code}** was not in the featured airports list.`,
    );
  }
}
...
async function handleListAirports(interaction: ChatInputCommandInteraction) {
  ...
  if (airports.length === 0) {
    await interaction.reply(
      "No featured airports yet. Use `/addairport` to add some.",
    );
    return;
  }
  await interaction.reply(
...
async function handleSetSchedule(
  ...
  await setScheduleChannel(channel.id);
  await interaction.reply(
...
async function handleTestSchedule(
  ...
  await postDailyRoutes(client, day);
}
...
export async function startBot() {
  ...
  registerCommands().catch((err) =>
    logger.error({ err }, "Failed to register Discord slash commands (non-fatal)"),
  );
  await Promise.race([
    loadScheduleChannel(),
    new Promise<void>((_, reject) =>
      setTimeout(() => reject(new Error("loadScheduleChannel timed out")), 5000),
    ),
  ]).catch((err) =>
    logger.warn({ err }, "Could not load schedule channel from DB (non-fatal)"),
  );
